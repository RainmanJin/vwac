...
{
    public partial class tmpcodelist : System.Web.UI.Page
    {
        ...
        private void BindList()
        {
            ...
            if (SearchName != "")
            {
                strWhere += " and (CardNumber like '%" + SearchName + "%')";
            }
            #endregion

            rpt_List.PageLink = "tmpcodelist.aspx?Search=Yes&SearchName=" + Utils.UrlEncode(SearchName);
            rpt_List.PageSize = 15;
            rpt_List.CurrentPage = LYRequest.GetInt("page", 1);

            DataSet ds = MW.BLL.mw_getlistbypage.GetDataSet_NoCache("mw_tempcode", "*", strWhere, "", "ID DESC", "ID", rpt_List.CurrentPage, rpt_List.PageSize, 1);
        ...
        {
            StringBuilder builder = new StringBuilder("tmpcodelist.aspx?Search=Yes");
