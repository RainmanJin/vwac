 ...
 * lhgdialog 核心代码部分
 */
var lhgdialog = function( config, ok, cancel,hidden)
{
	config = config || {};
	...
	config.hidden && config.button.push({
	    name: config.hiddenVal,
		callback: config.hidden
	});
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		DOM.hide[0].style.display = config.hidden? 'inline-block' : 'none';
	...
	button: function()
	{
		...
		buttons.style.display = list.length ? 'none' : 'none';
		that._ie6SelectFix();
	...
	_addEvent: function()
	{
		...
		DOM.wrap.bind('click', function(event){
			...
			else if( target === DOM.hide[0] || target === DOM.hide_b[0] )
			{
			    that._click(config.hiddenVal);
				return false;
			}
			else
			{
							...
							'<td colspan="2">' +
								'<div class="ui_title_bar">' +
									...
									'<div class="ui_title_buttons">' +
										'<a class="ui_hide" href="javascript:void(0);" title="\u6700\u5C0F\u5316"><b class="ui_hide_b"></b></a>' +
...
lhgdialog.setting =
{
	...
	hidden:null,                // 显示隐藏按钮回调函数
	hiddenVal:'\u6700\u5C0F\u5316'            // 隐藏按钮文本,默认'最小化'
};
