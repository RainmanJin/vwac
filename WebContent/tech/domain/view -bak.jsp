...
<form id="domainForm" action="${ctxPath}/techc/domain/save" method="post" class="form">
	...
	<input type="hidden" name="id" value="${dv.id}" />
		<ul>
			...
			<li><label>${i18n.domain.filed.value}</label><input <c:if test="${!(empty dv)}">readonly="readonly" class="gray"</c:if> type="text" name="value" value="${dv.domainvalue}" v="notEmpty" /><span class="msg">${i18n.domain.filed.value}${i18n.valid.notEmpty}</span></li>
