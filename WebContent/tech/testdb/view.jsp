	...
	function courseAjax() {
		...
		$.fn.jmodal({
					...
					content : function(body) {
						body.html('loading...');
						body.load('${ctxPath}/techc/commonresource/listTrainCourseModelByProTypeAndBrand?brand='
								...
								<c:choose>
									<c:when test="${not empty dto.list}">
										<c:forEach items="${dto.list}" var="db" varStatus="ids">
											<div class="rowAns">
												<input type="text" name="t_${ids.index}" value="${db.title}" /> <input style="width: 40px;" type="checkbox" name="c_${ids.index}"
													<c:if test="${db.isRight eq '1'}">checked="checked"</c:if> /> ${i18n.techdb.filed.isright}
											</div>
										</c:forEach>
									</c:when>
								</c:choose>
							</div> <span style="float: right;" class="btn"> <c:if test="${YON eq '1'}">
									<a href="javascript:addAnswer();">${i18n.oper.add} </a>
								</c:if>
						</span></li>
					</ul>
					<div class="btns" style="padding-left: 100px;">
						<span class="btn"> <c:if test="${YON eq '1'}">
