			...
			<li class="row">
				<dl>
					...
					<c:if test="${var.subitmeType eq '4'}">
						<textarea rows="5" cols="50" name="t_${var.id}">${var.content}</textarea>
					</c:if>
					<c:if test="${var.subitmeType eq '5'}">
						<d:radioDomain domain="QUESTIONTRAINCOM" name="c_${var.id}" value="${var.content}"/>
					</c:if>
					<c:if test="${var.subitmeType eq '1' || var.subitmeType eq '2' || var.subitmeType eq '3'}">
						<d:radioDomain domain="QUESTIONTYPE"  name="r_${var.id}" value="${var.content}"/>
					</c:if>
				</dl>
			</li>
			</c:forEach>
