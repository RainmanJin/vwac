...
#AnsDIV{
	...
	$(document).ready(function(){
		...
		function addAnswer()
		{
			...
			//$('#adddiv').append("<li><label>${i18n.techdb.filed.answer}</label><input name=\"+title_"+count+"\" id=\"aa\" value=\"${techanswer.title}\"/><span class=\"btn\"><a href=\"javascript:void(0);\" onclick=\"addAnswer();\">${i18n.oper.add}</a><span></li><li><label>${i18n.techdb.filed.isright}</label><label style=\"width:25px\" ><input name=\"techanswerright\" style=\"width:10px;\" type=\"radio\" id=\"techanswerright\" />${i18n.techdb.filed.yes}</label><label style=\"width:25px\" ><input name=\"techanswerright\" type=\"radio\" style=\"width:10px\" id=\"techanswerright\"/>${i18n.techdb.filed.no}</label></li>");
		}
						...
						<ul style="width: 700px">
							...
							<li>
								...
								<d:selectDomain domain="BRAND" uid="brand" name="brand"
									value="${dto.proType}" onchange="cleanCourse()" />
							</li>
							<li>
								<label>
							...
							<li>
								...
								<input id="courseNameId" onclick="courseAjax()" type="text" readonly="readonly" v="notEmpty" class="search" value="<c:if test="${!(empty dto.trainCourseId)}"><ecan:viewDto property="name" dtoName="TechTrainCourse" id="${dto.trainCourseId}"/><c:if test="${!(empty dto.moduleId)}">/<ecan:viewDto property="name" dtoName="TechTrainCourseItem" id="${dto.moduleId}"/></c:if></c:if>" />
							</li>
							
							
							<li style="height: auto;">
								<input name="testingId" id="techanswerid" type="hidden"
									value="${techanswer.id}" />
								<label>
								...
								<div id="AnsDIV">
									...
									<div class="rowAns">
										...
										<input style="width: 40px;" type="checkbox" name="c_1" />
										${i18n.techdb.filed.isright}
									</div>
									<div class="rowAns">
										<input type="text" name="t_2" />
										<input style="width: 40px;" type="checkbox" name="c_2" />
										${i18n.techdb.filed.isright}
									</div>
								</div>
